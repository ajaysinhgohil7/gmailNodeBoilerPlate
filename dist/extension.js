...
var Gmail = function(localJQuery) {
  ...
  api.get.delegated_to_email = function() {
    ...
    var delegatedToUserIndex = parseInt(
      pathname.substring(
    ...
    return $(
      ".gb_rb[href$='" + userIndexPrefix + delegatedToUserIndex + "'] .gb_yb"
    )
      .text()
      .split(" ")[0];
  };
  ...
  api.get.email_id = function() {
    console.warn(
      "GmailJS: api.get.email_id() invoked. Please note this function actually returns thread-id, and that email-id and thread-id may not always be used interchangably! Use api.get.thread_id() instead to silence this warning."
    );
  ...
  api.get.unread_inbox_emails = function() {
    var dom = $("div[role=navigation]").find(
      "[title*='" + api.tools.i18n("inbox") + "']"
    );
    const unread_inbox_mail = dom[0].getAttribute("aria-label");
    console.log(unread_inbox_mail);
    return unread_inbox_mail;
  };

  api.get.unread_draft_emails = function() {
    var dom = $("div[role=navigation]").find(
      "[title*='" + api.tools.i18n("drafts") + "']"
    );
    // console.log(dom[0]);
    // return dom[0].getAttribute("aria-label");
  ...
  api.get.unread_promotion_emails = function() {
    var dom = $("div[role=navigation]").find(
      "[title*='" + api.tools.i18n("promotions") + "']"
    );
  ...
  api.get.unread_social_emails = function() {
    var dom = $("div[role=navigation]").find(
      "[title*='" + api.tools.i18n("social_updates") + "']"
    );
    console.log(dom.length);
    if (dom.length > 0) {
      if (dom[0].title.indexOf("(") !== -1) {
    ...
    try {
      ...
      for (let fd_html_container of fd_html_containers) {
        fd_email_content_html =
          (fd_email_content_html || "") + fd_html_container["3"]["2"];
      }
    } catch (e) {
    ...
    try {
      ...
      for (let fd_thread_container of fd_threads) {
        ...
        for (let fd_email of fd_emails) {
          ...
          const fd_email_content_html = api.tools.parse_fd_request_html_payload(
            fd_email
          );
      ...
      console.warn(
        "Gmail.js encountered an error trying to parse email-data!",
        error
      );
      return null;
    }
  ...
  api.tools.parse_request_payload = function(params, events, force) {
    ...
    // console.log("Threads:");
    // console.log(threads);
    const emails = api.tools.extract_from_graph(
      params,
  ...
  api.tools.cache_email_data = function(email_data) {
    ...
    for (let threadId of threadIds) {
      ...
      if (firstEmail) {
        ...
        for (let email of emails) {
          if (
            thread.emails.filter(i => i.email_id === email.email_id).length ===
            0
          ) {
          ...
          this.onreadystatechange = function(progress) {
            if (this.readyState === this.DONE) {
              ...
              if (api.check.is_new_data_layer()) {
                if (
                  api.tools
                    .get_pathname_from_url(xhr.xhrParams.url_raw)
                    .endsWith("/i/fd")
                ) {
      ...
      recipient_change: {
        ...
        handler: function(match, callback) {
          ...
          setTimeout(function() {
            ...
            var compose = new api.dom.compose(
              api.tracker.recipient_matches[0].closest("div.M9")
            );
  ...
  api.observe.register = function(action, args) {
    ...
    if (typeof args === "object" && !$.isArray(args)) {
      ...
      $.each(["class", "selector", "sub_selector", "handler"], function(
        idx,
        arg
      ) {
        if (args[arg]) {
    ...
    if ($.inArray(action, api.tracker.supported_observers) > -1) {
      ...
      var timer = setInterval(function() {
        ...
        if (test > 0) {
          ...
          clearInterval(timer);
          console.log(
            "Failed to detect interface load in " +
              (delay * attempts) / 1000 +
              " seconds. Will automatically fire event in 5 further seconds."
          );
  ...
  api.helper.get.email_data_pre = function(thread_id) {
    console.warn(
      "Gmail.js: Usage of legacy email-data APIs have been deprecated by Google and will most likely fail. Migrate code to use gmail.new.get.email_data() to fix this problem."
    );
  ...
  api.get.email_source = function(email_id) {
    console.warn(
      "Gmail.js: This function has been deprecated and will be removed in an upcoming release! Please migrate to email_source_async!"
    );
  ...
  api.get.email_source_async = function(
    ...
    api.get
      .email_source_promise(email_id, preferBinary)
      .then(callback)
  ...
  extend(api.dom.compose.prototype, {
    ...
    recipients: function(options) {
      ...
      this.$el
        .find(".GS input[type=hidden]" + name_selector)
        .each(function(idx, recipient) {
          if (options.flat) {
  ...
  api.dom.email = function(element) {
    ...
    if (!element || !element.hasClass("adn"))
  ...
  api.dom.thread = function(element) {
    ...
    if (!element || !element.hasClass("if"))
